...
import { useGestaoControllerFindAll } from '@/api/generated'; // Adjust path if needed
...
const GestoresListScreen = () => {
  ...
  // Check if user is a logged-in gestor (assuming only gestores can manage other gestores)
  useEffect(() => {
  ...
  // Fetch all managers using the hook generated by Orval
  const { data: gestoresResponse, isLoading: isLoadingGestores, error: gestoresError, refetch } = useGestaoControllerFindAll({
    query: {
      enabled: !isLoadingUserCheck, // Only run query after user check is complete
    }
  });

  // Assuming the API returns { data: Gestor[] }
  ...
  if (isLoadingUserCheck || isLoadingGestores) {
    return (
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        {/* Add button to create new gestor if applicable */}
        {/* <TouchableOpacity 
          style={styles.createButton}
          onPress={() => router.push('/(home)/GestorCreateScreen')} // Adjust route as needed
        >
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              // Adjust route path and params for expo-router if a detail screen exists
              // onPress={() => router.push({ pathname: '/(home)/GestorDetailScreen', params: { gestorId: item.id }})}
            >
